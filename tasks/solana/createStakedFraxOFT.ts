...
task('lz:oft:solana:create:sfrax', 'Mints new SPL Token and creates new OFT Config account')
    ...
    .setAction(async (taskArgs: Args) => {
        ...
        const createTokenTx = await createAndMint(umi, {
            ...
            name: 'Staked Frax', // Token name
            symbol: 'sFRAX', // Token symbol
            isMutable: true, // Allow token metadata to be mutable
            decimals: LOCAL_DECIMALS, // Number of decimals for the token
            uri: 'https://arweave.net/4j88cRcNRTidWXjtAUUzQ7nrTImfeYj2dYWXQwGFSxU', // URI for token metadata
        ...
        fs.writeFileSync(`${outputDir}/sFraxOFT.json`, JSON.stringify(accountsJson, null, 2))
        console.log(`Accounts have been saved to ${outputDir}/sFraxOFT.json`)
    })
