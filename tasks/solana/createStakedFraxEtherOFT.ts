...
task('lz:oft:solana:create:sfrxeth', 'Mints new SPL Token and creates new OFT Config account')
    ...
    .setAction(async (taskArgs: Args) => {
        ...
        const createTokenTx = await createAndMint(umi, {
            ...
            name: 'Staked Frax Ether', // Token name
            symbol: 'sfrxETH', // Token symbol
            isMutable: true, // Allow token metadata to be mutable
            decimals: LOCAL_DECIMALS, // Number of decimals for the token
            uri: 'https://arweave.net/tZHV0Ptan6EmJ-e3wGNnHmDNJDiQkAMYc0nYf1ABciA', // URI for token metadata
        ...
        fs.writeFileSync(`${outputDir}/sfrxETHOFT.json`, JSON.stringify(accountsJson, null, 2))
        console.log(`Accounts have been saved to ${outputDir}/sfrxETHOFT.json`)
    })
