...
task('lz:oft:solana:create:fpi', 'Mints new SPL Token and creates new OFT Config account')
    ...
    .setAction(async (taskArgs: Args) => {
        ...
        const createTokenTx = await createAndMint(umi, {
            ...
            name: 'Frax Price Index', // Token name
            symbol: 'FPI', // Token symbol
            isMutable: true, // Allow token metadata to be mutable
            decimals: LOCAL_DECIMALS, // Number of decimals for the token
            uri: 'https://arweave.net/Sul8s40BObQXLmQrAm9ty0-_7bClEmX_ETl2xDUqKbg', // URI for token metadata
        ...
        fs.writeFileSync(`${outputDir}/FPIOFT.json`, JSON.stringify(accountsJson, null, 2))
        console.log(`Accounts have been saved to ${outputDir}/FPIOFT.json`)
    })
