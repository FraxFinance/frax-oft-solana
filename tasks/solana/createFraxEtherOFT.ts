...
task('lz:oft:solana:create:frxeth', 'Mints new SPL Token and creates new OFT Config account')
    ...
    .setAction(async (taskArgs: Args) => {
        ...
        const createTokenTx = await createAndMint(umi, {
            ...
            name: 'Frax Ether', // Token name
            symbol: 'frxETH', // Token symbol
            isMutable: true, // Allow token metadata to be mutable
            decimals: LOCAL_DECIMALS, // Number of decimals for the token
            uri: 'https://arweave.net/uFuPPo_gaEfp3AblJVdjiJlAQA-W7Gb-Rai5ZUgB5Bs', // URI for token metadata
        ...
        fs.writeFileSync(`${outputDir}/frxETHOFT.json`, JSON.stringify(accountsJson, null, 2))
        console.log(`Accounts have been saved to ${outputDir}/frxETHOFT.json`)
    })
