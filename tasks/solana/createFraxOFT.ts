...
task('lz:oft:solana:create:frax', 'Mints new SPL Token and creates new OFT Config account')
    ...
    .setAction(async (taskArgs: Args) => {
        ...
        const createTokenTx = await createAndMint(umi, {
            ...
            name: 'Frax', // Token name
            symbol: 'FRAX', // Token symbol
            isMutable: true, // Allow token metadata to be mutable
            decimals: LOCAL_DECIMALS, // Number of decimals for the token
            uri: 'https://arweave.net/oqdarB6_emvVFHA_QaJa4Amsy-YrFj8Fi4IqtWAmnqk', // URI for token metadata
        ...
        fs.writeFileSync(`${outputDir}/FraxOFT.json`, JSON.stringify(accountsJson, null, 2))
        console.log(`Accounts have been saved to ${outputDir}/FraxOFT.json`)
    })
